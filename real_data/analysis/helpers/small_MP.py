...
if __name__ == "__main__":
    ...
    df = pd.read_csv(msa_file, sep=r"\s+", dtype=str)
    #print(df)
    #msa = df.set_index("cell_name")["puro"].where(lambda x: x.notna(), None).to_dict()
    if "puro" in df.columns:
        msa = { row["cell_name"]: ([row["puro"]] if pd.notna(row["puro"]) else ["None"]) for _, row in df.iterrows() }
        print("num unique:", df['puro'].nunique())
    else:
        msa = { row["cell_name"]: ([row["blast"]] if pd.notna(row["blast"]) else ["None"]) for _, row in df.iterrows() }
        print("num unique:", df['blast'].nunique())
    #print(msa)

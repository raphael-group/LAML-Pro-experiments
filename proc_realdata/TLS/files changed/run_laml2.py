...
def main():
    ...
    if args["input_type"] == "character_matrix":
        charMtrx, site_names = read_sequences(args["characters"],filetype="charMtrx",delimiter=delimiter,masked_symbol=int(args["missing_data"]))
        K = len(charMtrx[next(iter(charMtrx.keys()))])
    elif args["input_type"] == "allele_counts":
        alleleTable_data_struct, alphabet_data_struct = read_sequences(args["characters"],filetype="alleleTab",delimiter=delimiter,masked_symbol=int(args["missing_data"]))
        # this is the number of cassettes
        K = len(alphabet_data_struct)
    elif args["input_type"] == "observed_features":
        charMtrx_data_struct, alphabet_data_struct = read_sequences(args["characters"],filetype="obsFeatures",delimiter=delimiter,masked_symbol=int(args["missing_data"]))
        K = len(alphabet_data_struct)
    else:
    ...
    if args['input_type'] == "character_matrix":
        ...
        print(alphabet_data_struct)
        alphabet = Alphabet(K,J,alphabet_data_struct)
        #print(alphabet.get_cassette_alphabet(0))
        DLT_data = AlleleTable(alleleTable_data_struct,alphabet)
        for k in range(K):
            allele_list = DLT_data.alphabet.get_cassette_alphabet(k)
            print(tuple([0]*J) in allele_list)
